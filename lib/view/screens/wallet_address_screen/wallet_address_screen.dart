...
class _WalletAddressScreenState extends State<WalletAddressScreen> {
  ...
  void showAlertDialog(BuildContext context) {
    final controller = Get.find<WalletAddressController>();
      ...
      builder: (BuildContext dialogContext) {
        return Obx(() {
          return AlertDialog(
            ...
            content: controller.isGeneratingAddress.value
                ? CircularProgressIndicator()
                : Text('Are you sure you want to generate a new address?'),
            actions: <Widget>[
              if (!controller.isGeneratingAddress.value) ...[
                TextButton(
                  child: Text('Cancel'),
                  onPressed: () {
    ...
    ever(controller.addressGenerationCompleted, (_) {
      if (controller.addressGenerationCompleted.value) {
        Navigator.of(context).pop();
        controller.resetAddressGenerationFlag();
      }
    });
  }
                            ...
                            children: [
                              ...
                              Obx(() => TextButton(
                                    style: TextButton.styleFrom(
                                      ...
                                      backgroundColor:
                                          controller.isGeneratingAddress.value
                        ...
                        controller.walletAddressList.isEmpty
                            ? const Expanded(
                                child: NoDataOrInternetScreen(
                                  message2: MyStrings.walletAddressEmptyMsg,
                                ),
                              )
                            : Flexible(
                                child: ListView.builder(
                                  ...
                                  itemBuilder: (context, index) {
                                    if (index ==
                                        controller.walletAddressList.length) {
                                      return Center(
                                    ...
                                    return WalletAddressListItem(
                                      model:
                                          controller.walletAddressList[index],
                                      cryptoImagePath:
                                          '${UrlContainer.baseUrl}${controller.imagePath}',
                                    );
                                  },
                                ),
                              ),
